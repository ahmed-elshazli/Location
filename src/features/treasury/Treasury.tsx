...
import { Plus, DollarSign, TrendingUp, TrendingDown, Search, Filter, Edit2, Trash2, ChevronLeft, ChevronRight } from 'lucide-react';
...
export default function Treasury() {
  ...
  const [agentFilter, setAgentFilter]               = useState('all'); // ← جديد
  ...
  const filtered = transactions.filter((tx: any) => {
    ...
    const matchAgent    = agentFilter    === 'all' || safeStr(tx.salesAgent) === agentFilter; // ← جديد
  ...
  const allAgents     = [...new Set(transactions.map((tx: any) => safeStr(tx.salesAgent)).filter(a => a !== '—'))]; // ← جديد
    ...
    <div className="p-6" dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      <div className="bg-white rounded-lg border border-[#E5E5E5] p-6 mb-6">
        <div className="flex flex-col gap-4">
          ...
          {showFilters && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 pt-4 border-t border-[#E5E5E5]">
              {/* Type */}
              <div>
              ...
              {/* ← Sales Agent filter جديد */}
              <div>
                <label className={`block text-sm font-medium text-[#16100A] mb-2 ${isRTL ? 'text-right' : 'text-left'}`}>{isAr ? 'مندوب المبيعات' : 'Sales Agent'}</label>
